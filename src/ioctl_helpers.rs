macro_rules! ioctl_param {
    ...
    ($ioctl_base:expr, $num:expr) => {
        libc::_IO($ioctl_base, $num)
    };
}
...
macro_rules! define_ioctl {
    ...
    ($(#[$meta:meta])* $fn_name:ident, $ioctl_num:expr, $ioctl_base:expr) => {
        $(#[$meta])*
        pub unsafe fn $fn_name(fd: libc::c_int) -> Result<(), libc::c_int> {
            let res =
            unsafe { libc::ioctl(fd, ioctl_param!($ioctl_base, $ioctl_num)) };
            if res != 0 {
                return Err( unsafe {* libc::__errno_location()});
            }
            Ok(())
