...
fn main() {
    ...
    let _ = unsafe { amdgpu_linux_api::drm::ioctl::drm_ioctl_set_master(file.as_raw_fd()) };
    ...
    let _ = unsafe { amdgpu_linux_api::drm::ioctl::drm_ioctl_set_master(file2.as_raw_fd()) };

    println!("Now there should be two clients, the new one should not be master");
    let _ = std::io::stdin().read_line(&mut String::new());

    let _ = unsafe { amdgpu_linux_api::drm::ioctl::drm_ioctl_drop_master(file.as_raw_fd()) };
